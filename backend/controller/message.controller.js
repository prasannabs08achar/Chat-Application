...
const getUserForSidebar=async(req,res)=>{
    try {
        ...
        const promises=filterdUser.map(async(user)=>{
            const messages=await Message.find({senderId:user._id,receiverId:userId,seen:false}).countDocuments();
            if(messages.length>0){
                unseenMessages[user._id]=messages.length;
            }
        })
        await Promise.all(promises);    
        return res.json({success:true, users:filterdUser, message:"Users fetched successfully"})
    } catch (error) {
...
const getMessages=async(req,res)=>{
    try {
        ...
        await Message.updateMany({senderId:userId,receiverId:selectedUserId},{seen:true});
...
const sendMessage=async(req,res)=>{
    try {
        ...
        let imageUrl;
        if(image){
            const imageUrl=uploadCloudinary(image);
